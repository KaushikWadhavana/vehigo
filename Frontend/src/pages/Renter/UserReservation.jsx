...
if (value === "pickup") {
  try {
    await axios.post(
      `http://localhost:5000/api/reservations/pay-pickup/${selectedReservation._id}`,
      {},
      { headers: { Authorization: `Bearer ${token}` } }
    );
...
try {
  await axios.post(
    "http://localhost:5000/api/reservations/verify-payment",
    {
...
const orderRes = await axios.post(
  `http://localhost:5000/api/reservations/create-order/${selectedReservation._id}`,
  {},
  { headers: { Authorization: `Bearer ${token}` } }
);
