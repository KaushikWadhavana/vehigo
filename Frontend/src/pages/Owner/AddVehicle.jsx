...
await axios.post(
  ...
  return (
    <div className="space-y-6 px-2 sm:px-4 md:px-6">
      ...
      <div className="bg-white border rounded-2xl p-4 sm:p-6 space-y-6">
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
        {FEATURES.map((f) => (
          <label key={f} className="border p-3 rounded flex gap-2">
            <input
              type="checkbox"
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
        {servicesData.map((s) => (
          <div
            key={s.key}
            className={`border rounded-xl p-4 flex gap-4 items-start cursor-pointer
              ${s.enabled ? "border-orange-400 bg-orange-50" : ""}
            `}
...
function EditExtraPricingModal({ services, setServices, onClose }) {
  ...
  return (
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
      <div className="bg-white w-full max-w-3xl rounded-2xl p-6 space-y-4 relative">
        {/* HEADER */}
        <div className="flex justify-between items-center">
...
function UploadsTab({ vehicleId, setActiveTab, setDraft, setCompleted, draft }) {
  ...
  const renderUploadBox = (title, docType) => (
    <label className="border-2 border-dashed rounded-xl p-6 flex flex-col items-center gap-2 cursor-pointer hover:bg-gray-50">
    ...
    <div className="space-y-6">
      ...
      <div className="bg-gray-50 rounded-xl p-4">
        <p className="font-medium mb-4">Total FAQ : {faqs.length}</p>
          ...
          {faqs.map(f => (
            <div key={f.tempId} className="bg-white border rounded-xl p-4 flex justify-between">
            <div>
  ...
  return (
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
      <div className="bg-white max-w-xl w-full p-6 rounded-2xl relative">
        ...
        <textarea
          className="w-full border rounded-lg mt-4 px-3 py-2"
          rows={4}
          placeholder="Answer"
  ...
  <div className="space-y-1">
    ...
    <input
      ...
      className={`w-full rounded-xl px-4 py-2.5 text-sm
        ...
        ${error ? "border-red-500 ring-1 ring-red-400" : "border-gray-200"}
      `}
    />
...
const YearSelect = ({ value, onChange, error }) => {
  ...
  return (
    <div className="relative">
      ...
      {open && (
        <div className="absolute z-50 bg-white border rounded-xl shadow-lg mt-2 p-4 w-full">
