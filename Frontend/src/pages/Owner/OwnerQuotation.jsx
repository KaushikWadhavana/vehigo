...
export default function OwnerQuotation() {
...
navigate(`/owner/reservationinfo/${r._id}`);
  ...
  // 🔥 ADD THIS LOGIC (LIKE USER PAGE)
  const updated = data.map(r => {
    const isLocked =
      r.approvalStatus === "approved" &&
      r.payment?.status !== "Paid" &&
      r.payment?.method !== null &&
      r.status !== "completed";
...
.filter((r) => {
  ...
  // 🔥 HANDLE APPROVAL STATUS (pending / approved)
  if (status === "pending" || status === "approved" || status === "rejected") {
    return r.approvalStatus === status;
  }

  // 🔥 HANDLE BOOKING STATUS (upcoming, completed, etc.)
...
{showAllFilters && (
  ...
  <tr>
    ...
    <th className="p-3 text-center w-[12%]">Pickup</th>
    <th className="p-3 text-center w-[12%]">Drop</th>
    <th className="p-3 text-center w-[8%]">Passengers</th>
    <th className="p-3 text-center w-[10%]">Total</th>
    <th className="p-3 text-center w-[10%]">Payment</th>
...
<td className="px-3 py-4 text-center align-middle whitespace-nowrap">
  ...
  {/* 🟢 ONLINE */}
  {r.payment?.method === "razorpay" && !r.isLocked && (
  ...
  {/* 🔵 PICKUP */}
  {r.payment?.method === "pickup" && !r.isLocked && (
    <>
      <span
        className={`px-3 py-1 text-xs rounded-full font-medium ${
          (r.payment?.status || "").toLowerCase() === "paid"
...
<td className="px-3 py-4 text-center align-middle">
  ...
  >
    {r.status === "in_rental" ? "In Rental" : r.status}
  </span>
</td>
...
<td className="px-3 py-4 text-center align-middle relative menu-container">
  ...
  {openMenu === r._id && (
    ...
    <div
  onClick={() => {
    handleView(r);   // 🔥 USE YOUR FUNCTION
    setOpenMenu(null);
  }}
...
{filteredReservations.length === 0
  ...
  {" "}to {Math.min(currentPage * itemsPerPage, filteredReservations.length)} of {filteredReservations.length}
  </p>
