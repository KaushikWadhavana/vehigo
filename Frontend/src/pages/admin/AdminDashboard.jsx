...
<button
  onClick={() => {
    ...
    if (v.addedBy === "owner") {
      navigate(`/admin/owners/${v.owner}/vehicle/${v._id}`);
    } else {
      navigate(`/admin/vehicle/${v._id}`);
    }
  }}
...
<div className="bg-white rounded-2xl p-5 border shadow-sm w-full lg:w-[420px] h-[420px] flex flex-col">
  ...
  src={
    c.image ||
    `https://api.dicebear.com/7.x/initials/svg?seed=${c.name || "User"}`
  }
...
<div className="bg-white rounded-2xl p-6 border shadow-sm flex-1 h-[420px] flex flex-col">
 ...
 <div className="flex justify-between items-start mb-4">
  ...
  {/* OPTIONAL (weekly below) */}
<p className="text-xs text-gray-400 mt-1">
  {range === "thisWeek"
    ? `This Week: ${data.formattedWeekly}`
    : `Last Week: ${data.formattedPrevWeekly}`}
</p>
</div>
...
<div className="bg-white rounded-2xl p-5 border shadow-sm mt-6">
  ...
  onClick={() => {
    if (b?.type === "reservation") {
      navigate(`/admin/reservationinfo/${b._id}`);
    } else {
      navigate(`/admin/booking/${b._id}`);
    }
  }}
