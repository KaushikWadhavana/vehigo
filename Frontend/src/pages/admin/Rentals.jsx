...
export default function OwnerUserPayments() {
...
// ✅ TOTAL BOOKINGS
const totalBookings = validPayments.length;
...
// ✅ COMBINE ALL VALID BOOKINGS
const vehicleMap = {};

validPayments.forEach((p) => {
  if (!p.vehicleName) return;

  vehicleMap[p.vehicleName] = (vehicleMap[p.vehicleName] || 0) + 1;
});

// ✅ SORT & GET TOP VEHICLE
const sortedVehicles = Object.entries(vehicleMap).sort((a, b) => b[1] - a[1]);

const topVehicleName = sortedVehicles[0]?.[0] || "N/A";
const topVehicleCount = sortedVehicles[0]?.[1] || 0;

const isBike = topVehicleName?.toLowerCase().includes("bike");
...
validPayments.forEach((p) => {
  ...
  if (m === currentMonth) currentVehicleCount++;
  if (m === lastMonth) lastVehicleCount++;
});
...
<div className="bg-white rounded-2xl p-5 border border-green-200 border-b-4 border-b-green-500 shadow-sm hover:shadow-md transition">
  ...
  <div className="flex justify-between items-center">
    ...
    {/* LEFT */}
    <div>
      <p className="text-gray-500 text-sm">Most Rented Vehicle</p>
...
<th className="p-3 text-left whitespace-nowrap">Booking</th>             </tr>
              </thead>
...
/>
                    ...
                    <td className="p-3 font-semibold whitespace-nowrap">₹{Number(p.amount || 0)}</td>
