...
const [reasonModal, setReasonModal] = useState({
  ...
  const load = async () => {
    ...
    const res = await fetch(
      "${import.meta.env.VITE_API_URL}/api/bookings/admin-bookings",
...
const handleEdit = () => {
  ...
  const save = async () => {
    if (!form.pickupLocation || !form.returnLocation) {
      return Swal.fire("Error", "Pickup & Return required", "error");
    }

    const token = await auth.currentUser.getIdToken();

    const res = await fetch(
      `${import.meta.env.VITE_API_URL}/api/bookings/admin-update/${id}`,
      {
        method: "PUT",
        headers: {
...
{b.status === "Rejected" && b.rejectReason && (
  ...
  <button
    ...
    className="absolute top-3 right-3 bg-white/90 px-4 py-1 rounded-xl text-sm shadow"
  >
    {edit ? "Save" : "Edit"}
  </button>
...
b.paymentFull?.method === "pickup" &&
    b.paymentFull?.status !== "Paid"   // 🔥 DISABLE IF PAID
  }
...
{reasonModal.open && (
  ...
  >
    ...
    {/* MODAL */}
    <div
      className="relative bg-white w-[90%] max-w-md rounded-3xl shadow-2xl p-6 animate-[fadeIn_0.25s_ease]"
    ...
    >
      ...
      {/* CONTENT */}
      <div className="bg-gray-50 border rounded-xl p-4 max-h-[200px] overflow-y-auto text-sm text-gray-700 leading-relaxed">
        {reasonModal.text}
      </div>
